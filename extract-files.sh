...
function blob_fixup() {
    case "${1}" in
        ...
        odm/etc/init/vendor-oplus-hardware-oplusSensor-V1-service.rc)
            ...
            ;;
        odm/lib/liblvimfs_wrapper.so|odm/lib64/libYTCommon.so|odm/lib64/libCOppLceTonemapAPI.so|odm/lib64/libaps_frame_registration.so|odm/lib64/libyuv2.so|vendor/lib64/libalsc.so)
            [ "$2" = "" ] && return 0
